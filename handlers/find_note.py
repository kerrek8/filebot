...
@router.message(F.text == '📋 Просмотр заметок')
async def start_views_noti(message: Message, state: FSMContext):
    await state.clear()
    kb = await find_note_kb()
    await message.answer('Выбери какие заметки отобразить', reply_markup=kb)
...
@router.message(F.text == '📅 По дате добавления')
async def date_views_noti(message: Message, state: FSMContext):
    await state.clear()
    all_notes = await get_notes_by_user(user_id=message.from_user.id)

    if all_notes:
        kb = await generate_date_keyboard(all_notes)
...
@router.callback_query(F.data.startswith('date_note_'))
...
@router.message(F.text == '📝 По типу контента')
async def content_type_views_noti(message: Message, state: FSMContext):
    await state.clear()
    all_notes = await get_notes_by_user(user_id=message.from_user.id)
    if all_notes:
        kb = await generate_type_content_keyboard(all_notes)
...
@router.callback_query(F.data.startswith('content_type_note_'))
...
@router.message(F.text == '🔍 Поиск по тексту')
...
@router.message(F.text, FindNoteStates.text)
